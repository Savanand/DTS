...
<script type="text/javascript" src="dwr/interface/Com_ser_dao.js"></script>
...
function initPage(){
	//alert("in it");
Com_ser_dao.getNewFileId(showNewId2);
...
<div id="menu">
	<ul id="main">
		...
		<li class="current_page_item"><a href="Common_server.jsp">Common Server</a></li>
...
<form id="frmUploadDoc" method="post" action="<%=request.getContextPath()%>/Common_ser?action=upload" enctype="multipart/form-data" name="frmUploadDoc">
<input type="hidden" name="file_id" id="file_id">
<input type="hidden" name="user_name" value="${sessionScope.user_logged }">
<table>
...
<h2>All Files</h2>
				...
				<c:when test="${sessionScope.admin_yes==1 }">
					...
					<sql:query var="qry" dataSource="${dataSource}">select DISTINCT file_id, file_name, file_size, file_desc,updated_by  from dts_common_server</sql:query>
					     		...
					     		<td><a href="<%=request.getContextPath()%>/Common_ser?file_id=${row.file_id }&action=remove">Remove</a></td>
					      		<td><a href="Common_ser?action=download&docName=${row.file_name}&docSize=${row.file_size}">download</a></td>
					      		</tr>
					    		</c:forEach>
